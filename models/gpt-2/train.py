...
from model import GPT2, TransformerConfig
...
def prepare_datasets(config):
    ...
    # Load dataset (example using tiny_shakespeare)
    ...
    def tokenize_function(example):
        ...
        if len(input_ids) > config.block_size:
            ...
            padding = [0] * (config.block_size - len(input_ids))
...
def evaluate(model_apply_fn, params, eval_data, config, num_batches=10):
    ...
    for _ in range(num_batches):
        ...
        # Forward pass
        logits = model_apply_fn(params, batch["input_ids"], training=False)
...
def train_step(state, batch, dropout_rng):
    ...
    def loss_fn(params):
        ...
        # Compute loss
        labels = jax.nn.one_hot(batch["target_ids"], state.params["wte"]["embedding"].shape[0])
        loss = jnp.mean(
            jax.nn.cross_entropy_with_logits(logits, labels) * 
            (batch["target_ids"] != 0)  # Mask out padding tokens
        )
...
def train_gpt2(config, num_epochs=5, steps_per_epoch=1000, save_every=1000):
    ...
    for epoch in range(num_epochs):
        ...
        for step in tqdm(range(steps_per_epoch)):
            ...
            if (step + 1) % save_every == 0:
                checkpoint_dir = f"checkpoints/gpt2_epoch{epoch+1}_step{step+1}"
...
def generate_text(model, params, tokenizer, prompt, max_new_tokens=100, temperature=1.0):
