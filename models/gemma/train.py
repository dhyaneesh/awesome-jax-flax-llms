    ...
    for epoch in range(num_epochs):
        ...
        for step in tqdm(range(steps_per_epoch)):
            ...
            if (step + 1) % save_every == 0:
                checkpoint_dir = f"checkpoints/gemma_epoch{epoch+1}_step{step+1}"
...
def load_model(checkpoint_dir, config):
    ...
    state = create_train_state(model, config, jax.random.PRNGKey(0))
    state = state.replace(params=restored["model"])
