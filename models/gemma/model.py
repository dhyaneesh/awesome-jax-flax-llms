...
def geglu(x, w1, w2, w3):
    ...
    return jnp.matmul(x, w1) * jax.nn.gelu(jnp.matmul(x, w2))
...
class GemmaMLP(nn.Module):
    ...
    def __call__(self, x):
        return self.w3(geglu(x, self.w1, self.w2, self.w3))
...
class GemmaBlock(nn.Module):
    ...
    def setup(self):
        self.attn = GemmaCausalSelfAttention(self.config)
...
class Gemma(nn.Module):
    ...
    def setup(self):
        ...
        self.layers = [GemmaBlock(self.config) for _ in range(self.config.n_layers)]
